...
// Базовый класс XmlParser
class XmlParser {
public:
    virtual void parse(const std::string& xmlfile) = 0; // чистая виртуальная функция
    virtual ~XmlParser() {}; // виртуальный деструктор
};

// Реализация парсера для Nmap XML
...
// Функция для парсинга XML-файлов теперь принимает вектор парсеров
void parse_xml_files_in_parallel(const std::vector<std::pair<XmlParser*, std::string>>& parser_file_pairs) {
    std::vector<std::thread> threads;

    for (const auto& pair : parser_file_pairs) {
        // Захватываем парсер и имя файла в лямбду
        threads.emplace_back([&, parser = pair.first, xmlfile = pair.second](){ parser->parse(xmlfile); });
    }
...
// Функция main теперь создает объекты NmapXmlParser и запускает параллельный парсинг
int main(int argc, char** argv) {
    ...
    std::vector<std::pair<XmlParser*, std::string>> parser_file_pairs;
    for (const std::string& file : xmlfiles) {
        NmapXmlParser* parser = new NmapXmlParser(output_directory);
        parser_file_pairs.emplace_back(parser, file);
    }

    // Запуск параллельного парсинга
    parse_xml_files_in_parallel(parser_file_pairs);

    // Освобождение ресурсов
    for (auto &pair : parser_file_pairs) {
        delete pair.first;
    }
